...
class GeneralFAQController extends Controller
{
    ...
    {
        $query = GeneralFaqs::query();
        if ($request->filled('title')) {
            $query->where(function ($q) use ($request) {
                $q->where('question', 'LIKE', '%' . $request->get('title') . '%');
                $q->orWhere('answer', 'LIKE', '%' . $request->get('title') . '%');
            });
        }
        if ($request->filled('category')) {
            $query->where('page', $request->get('category'));
        }
        $data = $query->paginate(10);
        return view('admin.faqs.lists', [
            'data' => $data
        ]);
    }
}
