...
<template>
    ...
    <BCollapse v-if="showButtons" :id="`collapse-${type}-${index}`" visible>
        <b-row class="gy-3 pt-3">
            <b-col v-for="(variant, index) in form.answers" cols="12">
                <b-row class="g-3">
                    ...
                    <b-col v-if="showButtons" cols="12" sm="4" md="4" lg="1" xxl="1">
                        <div class="d-flex text-end justify-content-end w-100 wc-md-84 wc-xl-96 gap-2">
                            <BaseButton
                                v-b-tooltip.hover
                                :title="`${$t('mark-correct-answer')}`"
                                :class="{'bg-white border-gainsboro': checkCorrectAnswer(form.answer , form.variants_answers[index])}"
                                :disabled="!showButtons"
                                @click="markCorrect(form.variants_answers[index])"
                            >
                                <template #text>
                                    <i :class="['ri-check-line', {'text-royal-blue': checkCorrectAnswer(form.answer , form.variants_answers[index])}]" />
                                </template>
                            </BaseButton>
                        </div>
                    </b-col>
